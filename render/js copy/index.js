...
devFolder.addColor(vars,"wireColor").onChange((e) => {
    if (vars.wireframeEnabled) {
        GroundMesh.material = new THREE.MeshBasicMaterial({
            color: vars.wireColor,
            wireframe: true
        });
    };
});

devFolder.add(vars,"wireframeEnabled").onChange((e) => {
    if (vars.wireframeEnabled) {
        GroundMesh.material = new THREE.MeshBasicMaterial({
            color: vars.wireColor,
            wireframe: true
    ...
    shapes.forEach(shape => {
        shape.wire.visible = vars.wireframeEnabled;
    });
});


const ChunkFolder = gui.addFolder("Chunks");
ChunkFolder.add(vars, "renderDistance", 1, 64,1).onChange(init)
ChunkFolder.add(vars, "chunkSize", 1,32,1).onChange(init)
ChunkFolder.add(vars, "LodFactor", 1,16,0.5).onChange(init)
ChunkFolder.open();
...
function onKeyPress(key) {
    switch (key) {
        ...
        case 'y': window.location.href = './Chunk.html'; break
    }
}
